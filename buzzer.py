...
pygame.init()
pygame.mixer.init()
pygame.mixer.music.load("videos/buzzer_123.wav")

screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
pygame.display.set_caption("Smart Traffic Signal GUI")
font = pygame.font.SysFont("Arial", 24)

# 🔸 SQL Logging Function (MySQL live insert)
def log_to_mysql(cycle, direction, green_time, total_vehicles):
    ...
    try:
        ...
        cursor = conn.cursor()
        query = """
            INSERT INTO traffic_log (cycle, direction, green_time, total_vehicles, date_time)
            VALUES (%s, %s, %s, %s, %s)
        """
        values = (cycle, direction, green_time, total_vehicles, timestamp)
...
class VehicleDetector:
    ...
    def count_vehicles(self):
        ...
        while time.time() - start_time < DETECTION_TIME:
            ...
            for cls, _, vehicle_id in detections:
                if vehicle_id not in seen_vehicle_ids:
                    seen_vehicle_ids.add(vehicle_id)
                    self.current_counts[cls] += 1
                    self.total_count += 1

        print(f"[{self.direction}] Unique Counts: {self.current_counts} | Total: {self.total_count}")
    ...
    def get_display_frame(self):
        ...
        cv2.putText(overlay, f"Total Count: {self.total_count}", (10, 60),
            ...
            for dir_label in system.fixed_order:
                ...
                for d in detectors:
                    ...
                    if d.direction == dir_label:
                        log_to_mysql(cycle_number, d.direction, green_time, d.total_count)
